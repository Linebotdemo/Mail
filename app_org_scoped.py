...
def load_user(user_id):
    ...
    try:
        cursor = db.cursor()
        cursor.execute('SELECT * FROM users WHERE id = ?', (user_id,)) AND tracking.organization_id = ?
...
def replace_links_with_tracking(html, employee_id, template_id):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if '/click/' in original_url:
            ...
            cursor.execute('SELECT url FROM tracking WHERE track_id = ?', (track_id,)) AND tracking.organization_id = ?
            row = cursor.fetchone()
            if row:
        ...
        cursor.execute('''
            SELECT * FROM tracking WHERE url = ? AND employee_id = ? AND template_id = ? AND tracking.organization_id = ?
...
def get_profile():
    try:
        ...
        if current_user.employee_id:
            ...
            cursor.execute('SELECT * FROM employees WHERE email = ?', (current_user.email,)) AND employees.organization_id = ?
            row = cursor.fetchone()
            if row:
...
def index():
    ...
    if user.role == "社員":
        ...
        cursor.execute("SELECT id FROM employees WHERE email = ?", (user.email,)) AND employees.organization_id = ?
        row = cursor.fetchone()
        ...
        if row:
            ...
            cursor.execute("""
                ...
                WHERE employee_id = ? AND employees.organization_id = ?
...
def portal_legacy():
    try:
        ...
        cursor.execute("SELECT id FROM employees WHERE email = ?", (current_user.email,)) AND employees.organization_id = ?
        row = cursor.fetchone()
        if not row:
        ...
        cursor.execute("""
            ...
            WHERE id IN ( AND employees.organization_id = ?
                SELECT template_id FROM signature_assignments
                WHERE employee_id = ? AND employees.organization_id = ?
                ORDER BY assigned_at DESC
                LIMIT 1
...
def auth():
    ...
    if request.method == 'POST':
        ...
        if not form.validate_on_submit():
            ...
            try:
                cursor = db.cursor()
                cursor.execute('SELECT * FROM users WHERE email = ?', (email,)) AND employees.organization_id = ?
                user = cursor.fetchone()
...
def get_employees(page=1, per_page=15, filter_name=None, filter_email=None, filter_department=None, filter_role=None, search=None, sort_by=None, sort_order='asc'):
    ...
    try:
        cursor = db.cursor()
        query = 'SELECT * FROM employees WHERE 1=1' AND employees.organization_id = ?
        params = []
        if filter_name:
            query += ' AND name LIKE ?'
...
def api_get_employee(id):
    ...
    try:
        cursor = db.cursor()
        cursor.execute('SELECT * FROM employees WHERE id = ?', (id,)) AND employees.organization_id = ?
...
def api_assign_signatures():
    ...
    try:
        ...
        for campaign in campaigns:
            ...
            cursor.execute("SELECT id FROM templates WHERE campaign_id = ?", (campaign_id,)) AND employees.organization_id = ?
            template_rows = cursor.fetchall()
            template_ids = [row['id'] for row in template_rows]

            # 部署に所属する社員を取得
            cursor.execute("SELECT id FROM employees WHERE department = ?", (department,)) AND employees.organization_id = ?
            ...
            for employee in employee_rows:
                ...
                for template_id in template_ids:
                    ...
                    cursor.execute("""
                        ...
                        WHERE employee_id = ? AND template_id = ? AND employees.organization_id = ?
...
def api_create_template():
    ...
    try:
        ...
        # HTMLを更新
        cursor.execute('UPDATE templates SET html_content = ? WHERE id = ?', (updated_html, template_id)) AND employees.organization_id = ?
        db.commit()
...
def api_get_template(id):
    ...
    try:
        cursor = db.cursor()
        cursor.execute('SELECT * FROM templates WHERE id = ?', (id,)) AND employees.organization_id = ?
...
def api_update_template(id):
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE id = ? AND employees.organization_id = ?
...
def api_delete_template(id):
    ...
    try:
        ...
        cursor.execute('DELETE FROM templates WHERE id = ?', (id,)) AND employees.organization_id = ?
        if cursor.rowcount == 0:
            logging.warning(f'⚠️ Template not found: id={id}')
            return jsonify({'success': False, 'message': 'テンプレートが見つかりません。'}), 404
        ...
        for row in cursor.fetchall():
            ...
            if tids != updated_tids:
                cursor.execute(
                    'UPDATE campaigns SET template_ids = ? WHERE id = ?', AND employees.organization_id = ?
...
def check_track_exists(track_id):
    ...
    try:
        cursor = db.cursor()
        cursor.execute('SELECT url FROM tracking WHERE track_id = ?', (track_id,)) AND employees.organization_id = ?
        row = cursor.fetchone()
...
def api_create_campaign():
    ...
    try:
        ...
        for tid in template_ids:
            cursor.execute('UPDATE templates SET campaign_id = ? WHERE id = ?', (campaign_id, tid)) AND employees.organization_id = ?
...
def api_track_click(track_id):
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE track_id = ? AND ip = ? AND tracking.organization_id = ?
        ...
        cursor.execute('UPDATE tracking SET clicks = clicks + 1 WHERE track_id = ?', (track_id,)) AND tracking.organization_id = ?
        cursor.execute('''
            INSERT INTO analytics (track_id, template_id, employee_id, ip, user_agent, created_at)
            SELECT track_id, template_id, employee_id, ?, ?, ?
            FROM tracking
            WHERE track_id = ? AND tracking.organization_id = ?
